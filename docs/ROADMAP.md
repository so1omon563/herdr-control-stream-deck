# Roadmap

## Release blockers

- Test upgrades, uninstall behavior, first-run permissions, and missing-Herdr errors.

## Later

- Parse and execute common custom Herdr keybindings for Spaces, Sidebar, Rename, and Close; the current fallback blocks affected actions when it detects an explicit override.
- Evaluate additional Stream Deck sizes from the same action set.
- Add Marketplace artwork and screenshots after the interaction model is stable.
- Add release automation only after a manual release is repeatable.
